...
import { CLICKED_DOWNLOAD_NOW_IOS_SECUREDIGITALSA, CLICKED_DOWNLOAD_NOW_SECUREDIGITALSA } from '@/events/EventName';
...
const AppStickyFooter = (props: AppStickyFooterProps) => {
    ...
    const handleEventClick = (event: string, platform: string = '') => {
        const gtmEventLabel = pathname === '/secure-digital-savings-account'
            ? `${platform}`
            : event;

        if (pathname === '/secure-digital-savings-account') {
            trackGTMEvent(gtmEventLabel);
        } else {
            trackEvent(event, { ...trackingData, method: 'manual', entry_point: 'home_page' });
            trackGTMEvent(event);
        }
    };

    const handleEventClickIos = (event: string) => {
        handleEventClick(event, CLICKED_DOWNLOAD_NOW_IOS_SECUREDIGITALSA);
    };

    const handleEventClickAndroid = (event: string) => {
        handleEventClick(event, CLICKED_DOWNLOAD_NOW_SECUREDIGITALSA);
    };
            ...
            <FixFooter isHomePageVariant={isHomePageVariant}>
                {
                    !isHomePageVariant ? (
                        <DownloadCTAWrapper>
                            <a
                                href={(windowState && windowState.oneLinkAndroidUrl) || playStoreUrl}
                                onClick={() => handleEventClickAndroid(CLICKED_GET_THE_FI_APP)}
                            ...
                            <a
                                href={(windowState && windowState.oneLinkIosUrl) || IOS_APP_STORE_URL}
                                onClick={() => handleEventClickIos(CLICKED_GET_THE_FI_APP_IOS)}
                        ...
                        <a
                            href={(windowState && windowState.oneLinkCommonUrl) || HOME_PAGE_VARIANT_ONE_LINK_URL}
                            id='poster_link'
                            target='_blank'
                            rel='noreferrer'
                        >
