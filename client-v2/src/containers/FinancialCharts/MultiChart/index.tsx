...
const rightPriceScaleConfig = (showYAxis: any) => ({
    entireTextOnly: true, borderVisible: false, drawTicks: false, visible: showYAxis,
});

const gridConfig = (showHorizontalLines: any) => ({
...
const localizationConfig = {
    priceFormatter: (p: any) => `${p.toFixed(2)}`,
};

const timeScaleConfig = (firstChartData: any, secondChartData: any) => ({
    timeVisible: true,
    secondsVisible: false,
    borderColor: Colors.CATSKILL_WHITE,
    tickMarkFormatter: (time: any) => {
        const chartDataFirstIndex = firstChartData.data.findIndex((d: any) => d.time === time);

        if (chartDataFirstIndex < 0) {
            const secondChartDataIndex = secondChartData.data.findIndex((d: any) => d.time === time);
...
const FinMultiChart = (props: any) => {
    ...
    */
    ...
    const setMultiChartData = (receivedData: any) => {
        setData(receivedData, 'setMultiChartData');
    };

    const setFreshChartData = (receivedData: any) => {
