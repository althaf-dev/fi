...
const calculateEPFInvestment = (args: EpfInvestmentArgs) => {
    ...
    const getEpfDepositValue = (counter:number) => {
    ...
    const getEmployeeShare = (counter:number, epfDeposit:boolean) => {
    ...
    const getTotalEmployeeShare = (employeeShareValue:number) => totalEmployeeShare + employeeShareValue;

    const getEmployerShare = (counter:number, epfDeposit:boolean) => {
        if (epfDeposit && counter > 1 && (counter - 1) % 12 === 0) {
    ...
    const getTotalEmployerShare = (employerShareValue:number) => totalEmployerShare + employerShareValue;

    const getInterestBearingBalance = (counter:number, epfDeposit:boolean) => {
        if (epfDeposit) {
            if (counter === 1) {
    ...
    const getSumOfOneYearInterestBearingBalance = (interestBearingBalance:number) => sumOfOneYearInterestBearingBalance + interestBearingBalance;

    const getInterest = (counter:number) => {
        if (counter % 12 === 0) {
    ...
    const getTotalInterestEarned = (interestValue:number) => totalInterestEarned + interestValue;

    return {
        updateEPFInvestmentValues: (counter:number) => {
...
const calculateSIPInvestment = (args: SIPInvestmentArgs) => {
    ...
    const getIncrementValue = (counter:number) => {
    ...
    const investedAmountValue = (counter:number) => {
        if (counter === 1) {
            return sipAmount;
        }
    ...
    const getPortfolioValue = (counter:number, investmentAmountValue:number) => {
        if (counter === 1) {
    ...
    return {
        updateSIPInvestmentValues: (counter:number) => {
