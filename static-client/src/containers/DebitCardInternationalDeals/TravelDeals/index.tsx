...
import { ALL_TRAVEL_OFFERS_DEEPLINK, WEB_CALLBACK_OBJECTS } from '../constants';
import { ClientPlatform, triggerMobileEvents } from '../utils';

import { TravelDealsSection, TravelImage, TravelImage2, DealsGrid, DealCard, DealImage, ViewAllDeals, ArrowIcon, TermsAndConditions, TermsAndConditions2 } from './styles';
...
export const TravelDeals: React.FC = () => {
  const deals = [
    ...
    { id: 5, image: DEBIT_CARD_TRAVEL_FEST_IMG_URL_MAP.TOURIST_ATTRACTION_OFFERS},
  ];
  ...
  return (
    <TravelDealsSection>
      <TravelImage src={DEBIT_CARD_TRAVEL_FEST_IMG_URL_MAP.BEACH_HOUSE_IMG} alt="beach house" />
      <TravelImage2 src={DEBIT_CARD_TRAVEL_FEST_IMG_URL_MAP.TRAVEL_DEALS_IMG} alt="travel deal" />
      <DealsGrid>
        {deals.map((deal) => (
          <DealCard key={deal.id}>
            <DealImage src={deal.image} alt={`Travel Deal ${deal.id}`} 
            style={
              deal.id === 4
