...
const SubTextComponent = (subItem: any) => {
    ...
    return item.external ? (
        <Subtext
            ...
            tagType='text'
        >
            <a href={getRefLink(item.url)}>{item.label}</a>
        </Subtext>
    ) : (
        <Subtext
            key={item.id}
                            ...
                            <>
                                ...
                                >
                                    <OverlayGridCard
                                        type={1}
                                        isFirstCard
                                        onClick = {onNavigate(getRefLink(APP_URLS.PRIVACY_PAGE))}
                                        font={(
                                    ...
                                    {
                                        !isSSR && Number(width) > WINDOW_SIZE.DESKTOP ? (
                                            <a aria-label='credit-card-page' href={getRefLink(APP_URLS.CREDIT_CARD_PAGE)} target='_blank' rel='noreferrer'>
                                                <OverlayGridCard
                                                    type={2}
                                                    font={(
                                            ...
                                            <OverlayGridCard
                                                type={2}
                                                onClick={onNavigate(getRefLink(NAVIGATION_URLS.FEES.url))}
                                                font={(
                                                    <Font font='h4VariantOne' tagType='text' color='FI_PINK'>
                                                        No Hidden
                                                        <br />
                                ...
                                <NavLinkParentGrid>
                                    {
                                        (!isSSR && Number(width) > WINDOW_SIZE.DESKTOP ? FOOTER_NAVIGATION : FOOTER_NAVIGATION_TAB)?.map((item) => (
                                            <NavigationGrid key={item.id}>
                                                {
                                                    item.menuItems.filter((filteredItem) => !filterUrlFromDynamicConfig(filteredItem.label, '')).map((subItem: any) => (
                                        ...
                                        <FooterLinkWrapper>
                                            {footerLinkData.map((item) => (
                                                <FooterLink key={item.id} font='pSmall' tagType='text' color='TEXT_GREY_1'>
                                                    {
                                                        item.external ? (
                                                            <a href={getRefLink(item.url)} target='_blank' rel='noreferrer'>
                                                                {item.label}
                                                            </a>
                                                        ) : (
                                                            <StyledLink href={getRefLink(item.url)}>
                                                                {item.label}
                                                            </StyledLink>
                                                        )
                                                    }
