...
interface ICarouselProps {
    ...
    /**
     * The class name to apply to the Carousel component.
     */
    className?: string;
    /**
     * The default scroll position value to apply when the component mounts.
     */
    defaultScrollPositionValue?: number;
}
...
const Carousel = (props: ICarouselProps) => {
    const {
        children, leftArrow, rightArrow, leftScrollValue, rightScrollValue, className,
        defaultScrollPositionValue,
    } = props;
    ...
    const onScroll = (direction: ScrollDirection) => () => {
        if (scrollableRef.current) {
            ...
            (scrollableRef.current as HTMLElement).scrollBy({
                left: scrollValue,
                behavior: 'smooth',
            });
        }
    ...
    useEffect(() => {
        ...
        if (scrollableContainer) {
            ...
            (scrollableContainer as HTMLElement).addEventListener('scroll', handleScroll);

            return () => {
                (scrollableContainer as HTMLElement).removeEventListener('scroll', handleScroll);
            };
        }
    ...
    useEffect(() => {
        const scrollableContainer = scrollableRef.current;
        if (scrollableContainer && defaultScrollPositionValue) {
            setTimeout(() => {
                (scrollableContainer as HTMLElement).scrollBy({
                    left: defaultScrollPositionValue,
                    behavior: 'instant',
                });
            }, 100)
        }
