...
const Wrapper = styled.div<{ visible: boolean; isCards: boolean }>`
  @media ${Device.tab} {
    ...
    grid-template-columns: ${(props) =>
      props.isCards ? "repeat(2, 1fr)" : "repeat(3, min-content)"};
...
const MenuPagesDropdown = (props: MenuPagesDropdownProps) => {
  const { visible, data, dynamicConfigInfo, onClick, dataTestId, isCards } =
    props;
              ...
              {item.menuItems.length > 0 && item?.url ? (
                <MenuItemLink>
                  {item?.external ? (
                    ...
                    <Link href={getRefLink(item?.url)}>{item.label}</Link>
                  )}
                </MenuItemLink>
              ) : (
                ...
                item.menuItems
                  ...
                  .map((featureItem: any) => (
                    ...
                    >
                      <StyledLink
                        isCards={!!isCards}
                        href={getRefLink(featureItem.url)}
