...
const TextSize = (props: TextProps & { theme: ThemeType }) => {
    const fontStyle = props.theme.typography && props.theme.typography[props.font || 'p'];
    return { fontStyle };
};

const TextColor = (props: TextProps & { theme: ThemeType }) => {
    const color = props.theme.color && props.theme.color[props.color || 'CHARCOAL_GREY'];
    return { color };
};

const StyledText = styled.div<TextProps & { theme: ThemeType }>`
...
const StyledH1 = styled.h1<TextProps & { theme: ThemeType }>`
    color: ${(props) => TextColor(props).color};
    font-size: ${(props) => TextSize(props).fontStyle.phone.fontSize};
    font-weight: ${(props) => TextSize(props).fontStyle.phone.fontWeight};
    line-height: ${(props) => TextSize(props).fontStyle.phone.lineHeight};
