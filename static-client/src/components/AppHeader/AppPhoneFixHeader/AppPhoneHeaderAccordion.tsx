...
const SubheadingComponent = (props: any) => {
    ...
    return navItem.external ? (
        <SubHeading key={navItem.id} isFooter={isFooter}>
            <a href={getRefLink(navItem.url)}>{navItem.label}</a>
        </SubHeading>
    ) : (
        <SubHeading
        ...
        >
         ...
         TODO:
         */}
            <Link href={getRefLink(navItem.url)} onClick={onClick}>
                {navItem.label}
            </Link>
        </SubHeading>
    );
...
const HeaderAccordion = (props: AppPhoneAccordionProps) => {
    ...
    if (data.external) {
        linkElement = <a href={getRefLink(data.url)}>{data.label}</a>;
    } else {
        // TODO
        linkElement = <Link href={getRefLink(data.url)}>{data.label}</Link>;
    }
                        ...
                        )}
                        {
                            navItem.menuItems?.filter((item: any) => !filterUrlFromDynamicConfig(item.label, dynamicConfigInfo)).map((navSubItem: any) => (
                                <NavLinkSubHeadings
                                    ...
                                    href={getRefLink(navSubItem.url)}
                                    isFooter={isFooter}
                                    onClick={onClick}
                                >
                                    {navSubItem.label}
