...
const transformUSStocksData = (usStocksData: any[]): IUSStockData => {
    ...
    usStocksData?.forEach((stock) => {
        ...
        data[stockId] = {
            ...
            percentageReturns: {
                icon,
                text: fontColor && tagText ? `<span style="color: ${fontColor}">${tagText}</span>` : null,
            },
            symbol,
 ...
 * @returns {Array} - The transformed data.
 */
const transformUSStocksCollectionsData = (collectionsData = [], collectionDetails = [], collectionId) => {
    const filteredCollectionData = collectionsData.filter((collection) => collection.collection_id !== 'ALL_ETFS' && collection.collection_id !== 'ALL_STOCKS');
 ...
 */
const usStockTooltipLabelFn = (graphType, sumData?: number) => (context) => {
 ...
 */
const transormFinancialsRatioCardGraphData = (dataPoints) => {
    const lineChartData = {
        labels: [],
        datasets: dataPoints?.map(({ series_data_points: seriesDataPoints, series_color: seriesColor }) => ({
            label: false,
            data: seriesDataPoints.map(({ value }) => value),
    ...
    lineChartData.labels = dataPoints[0]?.series_data_points?.map(({ category_name: categoryName }) => categoryName) || [];
    ...
    const data = financialRatioTabs?.map((metricTab) => {
        ...
        const details = financialRatioCharts?.map((metricCharts) => {
            ...
            return {
                ...
                visuals: transormFinancialsRatioCardGraphData(multiRatioDataPoints) || {},
            };
        });
...
const transformFinancialsCardGraphData = (dataPoints) => {
    const financialsCardValues = dataPoints?.map((dataPoint) => dataPoint?.value / ONE_BILLION);
    const financialsCardBackgroundValues = dataPoints?.map((dataPoint) => dataPoint?.bar_color || '#5684AE');
    ...
    const visuals = {
        ...
        graphData: {
            labels: dataPoints?.map((dataPoint) => dataPoint?.category_name),
            datasets: [
                ...[getGraphDatasets({
...
const transformFinancialsCardData = (cardData) => {
    ...
    const data = financialMetricTabs?.map((metricTab) => {
        ...
        const details = financialMetricCharts?.map((metricCharts) => {
            ...
            return {
                bottomTag: reportingFrequency?.plain_string,
                visuals: transformFinancialsCardGraphData(metricDataPoints),
            };
        });
 ...
 */
const transfromSymbolDesisionFactorsRpcResponse = (data) => {
    const { symbol_decision_factors_cards: symbolDecisionFactorsCards } = data || {};

    const transformArrayIntoCardObject = symbolDecisionFactorsCards?.reduce((acc, item) => {
