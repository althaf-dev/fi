...
export const getMiddleElementInList = (parentElement): HTMLDivElement => {
    const { children } = parentElement;
    return children[Math.floor((children?.length - 1) / 2)];
};
...
export const getDefaultMiddleScrollLeftOffSet = (parentElement) => {
