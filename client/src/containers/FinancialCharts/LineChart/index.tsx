...
const FinLineChart = (props) => {
    ...
    const [chartProperties, setChartProperties] = useState({
        ...
        timePeriod: '1D',
        isToolTooltipInitializeFromMax: false,
    });
    ...
    useEffect(() => {
        ...
        if (chartData.length > 0) {
            ...
            const completedTime = chartData[chartData.length - 1].time - Date.parse(formattedStartDate) / 1000;
        ...
        const handleResize = () => {
            chart.applyOptions({ width: chartContainerRef?.current?.clientWidth - rightOffset });
        };
        ...
        if (chartProperties.isToolTooltipInitializeFromMax) {
    ...
    const setFreshData = (receivedData:Array<Object>) => {
    ...
    const updateNewData = (stringifiedNewData: Object) => {
        ...
        try {
            ...
            setChartData((prevChartData) => {
                ...
                const isTimeAlreadyExists = prevChartData.some((item) => item.time === newData.time);
    ...
    const setInitialProperties = (properties) => {
        setChartProperties({
            ...
            isToolTooltipInitializeFromMax: properties.isToolTooltipInitializeFromMax,
        });
    };
