...
import { trackingPayload } from '../../Faq/constants';
...
const RenderSingleCalculator = (props: RenderSingleCalculatorProps) => {
    ...
    if (calcNameParam === CALCULATOR_URLS_MAPPING.FIRE_CALCULATOR) {
        graphBottomCalculatorInputFields = calculatorInputFields?.filter((data) => data.show_bottom_of_graph);
    }
    ...
    const getInputLabelComponent = (item) => {
    ...
    const setNewInputValue = (item) => (newValue): void => {
        dispatch(setInputValueAction({ item, value: newValue }));
    };

    const setOutputValue = (item) => (newValue): void => {
        dispatch(setOutputValueAction({ item, value: newValue }));
    };

    const getInputComponent = (item, hasMultiInput?: boolean) => {
        const {
    ...
    const getSliderComponent = (item, showComponentInsideStickyGraph?: boolean) => {
        const {
    ...
    const getMultiSliderComponent = (item) => {
        const {
            min_value: minValue, max_value: maxValue, value, debounce_time: debounceTime, step_value: stepValue,
    ...
    const getDropDownComponent = (item, hasDropDownWithSearch ?: boolean) => {
    ...
    const getCheckboxComponent = (item) => {
    ...
    const getAccordionComponent = (item, renderComponentBasedOnComponentTypeFuntion) => {
        const { accordion_label: accordionLabel, nested_items: nestedItems } = item;

        const accoridonElements = [];

        if (nestedItems?.length) {
            nestedItems.forEach((data) => {
    ...
    const getTextLableWithIconComponent = (item) => {
    ...
    const getErrorValue = (error, item) => {
        let errorValue;
    ...
    const renderComponentBasedOnComponentType = (item, index?: number, showComponentInsideStickyGraph?: boolean) => {
        let component;
        let componentOne;
    ...
    const renderMainCalculatorSection = (data) => (
        data?.map((item) => (
    ...
    const mainCalculatorSection = (
        <>
            ...
            <RightSection>
                {graphSection}
                {calculatorCtaInfo?.is_side_banner === 'right' && (
                    <CalculatorBanner
                        bannerData={calculatorCtaInfo}
                        showOnDesktop
                        sideBanner
    ...
    const calculatorFAQSSection = (
        <AdditionalInformationWrapper>
            <AdditionalInformationContainer>
                ...
                {calculatorAdditionalInfo?.map((item) => (
                    <AdditionalInformationSection key={item.input_id}>
                        <Title>
                            <div
                                // eslint-disable-next-line react/no-danger
    ...
    if (hasMobileDesignV1) {
        const {
            name, description, mobile_icon_url: mobileIconUrl, mobile_fallback_icon_url: mobileFallbackIconUrl,
...
const SingleCalculator = (props: SingleCalculatorProps) => {
    ...
    if (isValidSingleCalcUrl(calcNameParam) || internalPageUrl) return <RenderSingleCalculator internalPageUrl={internalPageUrl} />;
