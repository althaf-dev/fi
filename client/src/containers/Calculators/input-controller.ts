...
const rentVsBuyInputFn = (draft) => {

};

const epfInputFn = (state, draft, updatedInputValues, item, value) => {
    ...
    if (item.input_id === '5b1ed889-92b3-4aeb-8e7c-2dd5b0105a25') {
        const retirementAgeItemIndex = draft.singleCalculator.input_fields
            .findIndex((arrayItem) => arrayItem.input_id === '527c035b-0157-4eac-be3e-34d0a7f15919');
...
const fireInputFn = (state, draft, updatedInputValues, item, value) => {
    ...
    if (item.input_id === 'a1d08ee4-6f48-4261-9891-03ecf8bffa04') {
        const fireAgeItemIndex = calculatorInputFields.findIndex((arrayItem) => arrayItem.input_id === '35c0391f-35e1-4e8e-a5f1-ff2d93205b7b');
    ...
    if (lifestyleUpgradMultiplierInputIds.includes(item.input_id)) {
        const lifestyleUpgradeMultiplierItemIndex = calculatorInputFields.findIndex((arrayItem) => arrayItem.input_id === 'feb12f28-50c1-4dcf-87c8-0f61b3611891');
        const nestedItemIndex = calculatorInputFields.findIndex((arrayItem) => arrayItem.input_id === '8375c4c3-87c3-4a4e-beef-230f773d5b8d');
        const nestedItemObj = calculatorInputFields[nestedItemIndex];
        const expenseItemIndex = nestedItemObj.nested_items.findIndex((arrayItem) => arrayItem.input_id === '3a9ad9d7-f285-4a63-8464-7ba3b5f6288e');

        const lifestyleUpgradMultiplierNestedInputItems = calculatorInputFields
            .filter((data) => data.nested_items);

        const checkboxItems = lifestyleUpgradMultiplierNestedInputItems[0].nested_items
            .filter((data) => (lifestyleUpgradMultiplierInputIds.includes(data.input_id)));

        const lifestyleUpgradeMultiplierUpdatedValue = checkboxItems
            .reduce((accumulator, currentItem) => accumulator + currentItem.value.value, INITIAL_VALUE);
    ...
    if (item.input_id === 'feb12f28-50c1-4dcf-87c8-0f61b3611891') {
        ...
        const updatedNestedCheckboxItems = nestedInputItems[0].nested_items.map((nestedItem) => {
