...
export const selectAllCalculators = () => createSelector(
    (state) => state[CALCULATORS_KEY].allCalculators,
    (subState) => subState,
);

export const selectSingleCalculator = () => createSelector(
    (state) => state[CALCULATORS_KEY].singleCalculator,
    (subState) => subState,
);

export const selectCalculatorInfo = () => createSelector(
    (state) => state[CALCULATORS_KEY].singleCalculator?.calculator_info,
    (subState) => subState,
);

export const selectCalculatorCtaInfo = () => createSelector(
    (state) => {
        console.log("calc state",state[CALCULATORS_KEY])
       return state[CALCULATORS_KEY].singleCalculator?.banner_details
    },
    (subState) => subState,
);

export const selectCalculatorInputFields = () => createSelector(
    (state) => state[CALCULATORS_KEY].singleCalculator?.input_fields,
    (subState) => subState,
);

export const selectCalculatorAdditionalInfo = () => createSelector(
    (state) => state[CALCULATORS_KEY].singleCalculator?.additional_information,
    (subState) => subState,
);

export const selectCalculatorFaqsInfo = () => createSelector(
    (state) => state[CALCULATORS_KEY].singleCalculator?.faqs_information,
    (subState) => subState,
);

export const selectCalculatorOutputResult = () => createSelector(
    (state) => state[CALCULATORS_KEY].outputResult,
    (subState) => subState,
);
...
export const selectCalculatorGraphData = () => createSelector(
    (state) => state[CALCULATORS_KEY].outputResult?.visuals[0]?.data?.graphData,
    (subState) => subState,
);
