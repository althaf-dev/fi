...
interface GraphSectionProps {
    ...
    renderComponentBasedOnComponentType?: (item: any, index?: number, showComponentInsideStickyGraph?: boolean) => any;
}

const GraphSection = (props: GraphSectionProps) => {
    const {
        commonGraphSectionProps, stickSectionVisualType, isStickySection, graphBottomCalculatorInputFields, renderComponentBasedOnComponentType,
    } = props;

    const {
        graphView, outputResult, outputSentence, getVisualType, onVisible,
        visualType, graphType, graphData, graphInfo, viewVisualSection, graphOptions,
    ...
    const tableSection = (
        <TableDetails
            tableData={viewVisualSection && outputResult?.visuals[1]?.data.values}
        ...
        >
            ...
            {viewVisualSection ? (
                <VisualsContainer stickySection={isStickySection}>
                    {
                        outputResult?.visuals.map((item) => (
                            !isStickySection ? (
                                <VisualsSection
                                    key={item.label}
                                    onClick={getVisualType(item.type, isStickySection)}
                                ...
                                <StickyVisualSection
                                    key={item.label}
                                    onClick={getVisualType(item.type, isStickySection)}
                                    activeGraphLabelId={stickSectionVisualType === item.type}
                                >
                                    {item.label}
                                </StickyVisualSection>
                            )
            ...
            {graphBottomCalculatorInputFields?.length
                ? (
                    <StickyGraphBottomInputFields>
                        {graphBottomCalculatorInputFields.map((item) => (
                            <React.Fragment key={item.id}>
                                {renderComponentBasedOnComponentType(item, null, true)}
                            </React.Fragment>
                        ))}
                    </StickyGraphBottomInputFields>
                )
