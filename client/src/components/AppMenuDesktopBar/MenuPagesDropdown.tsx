...
const StyledLinkExternal = styled.span`
    display: inline-block;
    padding: 12px 0;
    width: 100%;
    text-transform: uppercase;
                        ...
                        >
                            ...
                            )}
                            {
                                item.menuItems && item.menuItems
                                    .filter((filteredItem) => !filterUrlFromDynamicConfig(filteredItem.label, dynamicConfigInfo))
                                    .map((featureItem) => (featureItem.external ? (
                                        <StyledLinkExternal key={featureItem.id} data-test-id={dataTestId + featureItem.dataTestId}>
                                            <a href={featureItem.url}>{featureItem.label}</a>
                                        </StyledLinkExternal>
                                    ) : (
                                        <StyledFont
                                            font='labelVariantThree'
                                            ...
                                            onClick={onClick}
                                        >
                                            <StyledLink isCards={!!isCards} to={featureItem.url} data-test-id={dataTestId + featureItem.dataTestId}>
                                                {featureItem.label}
                                            </StyledLink>
                                        </StyledFont>
                                    )))
