...
import { selectDynamicConfig } from '../../containers/App/selectors';
import { CONSUL_META_INFO_PATH_NAME } from '../../containers/App/constants';

import OverlayGridCard from './OverlayGridCard';
import {
    FI_PRONUNCIATION_DATA, APP_ASSISTANCE_DATA, SOCIAL_MEDIA_LINK_DATA, FOOTER_LINK_DATA, PATHS_TO_SHOW_SEBI_REGIONAL_OFFICE_ADDRESS, PATHS_TO_SHOW_STOCK_DIRECTORY,
} from './constants';
...
import HeaderAccordionCr from '../AppPhoneFixHeader/AppPhoneHeaderAccordionContainer';
...
const SubTextComponent = (subItem) => {
...
const AppFooter = (props: AppFooterProps) => {
    ...
    const showUsStockDirector = useMemo(() => PATHS_TO_SHOW_STOCK_DIRECTORY.some((route) => pathname.includes(route)),
        [pathname]);
    ...
    const {
        trackingPayload, noPadding, showSecondaryContent,
        screenName, parentComponent, salaryProgram,
    } = props;
    const {
    ...
    const onNavigate = (defaultUrl: string, alternateUrl?: string) => () => {
        if (alternateUrl && !isSSR && width < WINDOW_SIZE.DESKTOP) {
    ...
    return (
        <>
            {
                showUsStockDirector ? (<StockDirectory />
                ) : null
            }
                    ...
                    >
                            ...
                            !salaryProgram ? (
                                <>
                                    ...
                                    >
                                        ...
                                        {
                                            !isSSR && width > WINDOW_SIZE.DESKTOP ? (
                                                <a href={APP_URLS.CREDIT_CARD_PAGE} target='_blank' rel='noreferrer'>
                                                    <OverlayGridCard
                                                        type={2}
                                                        font={(
                                    ...
                                    <NavLinkParentGrid>
                                        {
                                            (!isSSR && width > WINDOW_SIZE.DESKTOP ? FOOTER_NAVIGATION : FOOTER_NAVIGATION_TAB)?.map((item) => (
                                                <NavigationGrid>
                                                    {
                        ...
                        <SecondRowContainer>
                            <SecondRowFirstColumnWrapper>
                                <FiLogo>
                                    <Image loadingType='lazy' icon={LOGOS_URL_MAP.FI_LOGO_FOOTER} />
                                </FiLogo>
