...
const NavLinkSubHeadingsExternal = styled.a<{ isFooter?: boolean }>`
    font-family: Gilroy, sans-serif;
    text-transform: ${(props) => (props.isFooter ? 'uppercase' : 'capitalize')};
    font-weight: ${(props) => (props.isFooter ? '700' : '500')};
    display: inline-block;
...
const HeaderAccordion = (props: AppPhoneAccordionProps) => {
    ...
    if (data.external) {
        ...
        linkElement = <Link to={data.url}>{data.label}</Link>;
    }
            ...
            <Container>
                ...
                {data.menuItems?.map((navItem) => (
                    ...
                    >
                        ...
                        {
                            navItem.menuItems?.filter((item) => !filterUrlFromDynamicConfig(item.label, dynamicConfigInfo)).map((navSubItem) => (
                                (navSubItem.external) ? (
                                    <NavLinkSubHeadingsExternal
